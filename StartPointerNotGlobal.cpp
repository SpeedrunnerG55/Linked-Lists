...
inline void displayInfo(Ptr temp1){
  printLine('-');
  //initialise output
  string outString;
...
// these do not modify pointer values but display and manipulate
// information contained in nodes
inline SearchResults createNode(Ptr start_ptr); //list stack and que
inline bool empty              (Ptr start_ptr); // used in list and stack
void first                     (Ptr start_ptr); //used in que and stack
void Display_List              (Ptr start_ptr); //displays whole list
inline void displayInfo        (Ptr start_ptr,Ptr temp1); //list stack and que
...
void add_start_node            (Ptr &start_ptr); // used in list and stack
void add_To_Middle             (Ptr &start_ptr); // list
void delete_start_node         (Ptr &start_ptr); //list stack and que
void add_node_at_end           (Ptr &start_ptr); //used in queue
...
int main() {
  ...
  do{
    ...
    switch (Menue) {
      case 1:
      switch (type) {
        case 0: add_To_Middle(List_ptr);   break;
        case 1: add_node_at_end(Queue_ptr); break;
        case 2: add_start_node(Stack_ptr);  break;
      }
      break;
      case 2:
      case 3:
...
inline SearchResults createNode(Ptr start_ptr){
  int ID;
...
void add_start_node(Ptr &start_ptr){
  Ptr temp1 = createNode(start_ptr).temp1;
  if(empty(start_ptr)){
    start_ptr = temp1;
  }
  else{
...
void add_To_Middle(Ptr &start_ptr){
  if(empty(start_ptr)){
    add_start_node(start_ptr);
  }
  else{
    Ptr temp1, temp2;
...
void add_node_at_end (Ptr &start_ptr){
  Ptr temp1 = createNode(start_ptr).temp1;
...
inline bool empty(Ptr start_ptr){
